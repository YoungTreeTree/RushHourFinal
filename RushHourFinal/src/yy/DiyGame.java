...
public class DiyGame extends Panel implements MouseListener,MouseMotionListener,ActionListener{
	...
	public DiyGame() {
		...
		jButton_save.setBounds(870, 310, 100, 50);
	...
	public void paint(Graphics g) {
		...
		/**************左边的例子*************/
		g.drawImage(image1[2],25,30,150,50,null,null);
		g.drawImage(image2[3],25,150,50,150,null,null);
		g.drawImage(image1[4],25,400,100,50,null,null);
		g.drawImage(image2[5],25,500,50,100,null,null);
	...
	private void drawMoveCars(Graphics g) {
		...
		int width=0;
		int height=0;
		Image image=Toolkit.getDefaultToolkit().getImage("src/background.jpg");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(src==jButton_save){
			int num=6-diyGames.getCarList().size();
			for(int i=0;i<num;i++){
				Car car=new Car();
			...
			new FileReader().writeGame("src/DiyCarPosition.txt", diyGames);
		}
	}
}
